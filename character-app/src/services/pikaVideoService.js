...
function getJobId(payload) {
  return payload.id || payload.job_id || payload.request_id
}

function getVideoUrl(payload) {
  return payload.video_url || payload.url || payload.result?.url || payload.output?.url || payload.data?.video_url
}
...
export async function generatePikaVideo(input) {
  ...
  while (Date.now() < deadline) {
    ...
    if (['failed', 'error', 'cancelled', 'canceled'].includes(state)) {
      throw new Error(status.error || status.message || 'Pika video generation failed')
    }
    const videoUrl = getVideoUrl(status)
    if (videoUrl) return videoUrl
    if (['completed', 'succeeded', 'success', 'done'].includes(state)) {
      return `${apiUrl}/v1/videos/${encodeURIComponent(jobId)}/content`
    }
  }
